  ...
  async (order) => {
    try {
      ...
      const { data } = await axios.post(
        "https://sixpackproject-qnef.onrender.comapi/v1/order/new",
        order,
        config
      );
...
export const myOrders = createAsyncThunk("user/myOrders", async () => {
  try {
    ...
    const { data } = await axios.get(
      "https://sixpackproject-qnef.onrender.comapi/v1/orders/me",
      config
    );
...
export const getOrderDetailsById = createAsyncThunk(
  ...
  async (id) => {
    try {
      ...
      const { data } = await axios.get(
        `https://sixpackproject-qnef.onrender.comapi/v1/order/${id}`,
        config
      );
...
export const fetchAdminAllOrders = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      const { data } = await axios.get(
        "https://sixpackproject-qnef.onrender.comapi/v1/admin/orders",
        config
      );
  ...
  async (id) => {
    try {
      ...
      const { data } = await axios.delete(
        `https://sixpackproject-qnef.onrender.comapi/v1/admin/order/${id}`,
        config
      );
