...
export const updateUserPassword = createAsyncThunk(
  ...
  async (userPasswordsData, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.put(
        "https://sixpackproject-qnef.onrender.com/api/v1/password/update",
        userPasswordsData,
        config
      );
  ...
  async (userMailingInfo, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.post(
        "https://sixpackproject-qnef.onrender.com/api/v1/password/forgot",
        userMailingInfo,
        config
      );
...
export const resetUserPassword = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `https://sixpackproject-qnef.onrender.com/api/v1/password/reset/${token}`,
        userPasswordsData,
        config
      );
      // saving token in coockie for 5 days
