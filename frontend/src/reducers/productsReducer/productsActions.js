  ...
  async () => {
    try {
      ...
      const { data } = await axios.get(
        "https://sixpackproject.onrender.comapi/v1/admin/products",
        config
      );
  ...
  async (id) => {
    try {
      ...
      const { data } = await axios.delete(
        `https://sixpackproject.onrender.comapi/v1/admin/product/${id}`,
        config
      );
...
export const fetchProductById = createAsyncThunk(
  ...
  async (id) => {
    try {
      const { data } = await axios.get(
        `https://sixpackproject.onrender.comapi/v1/product/${id}`
      );
      return data.product;
    } catch (err) {
      return err.message;
  ...
  async (product) => {
    try {
      ...
      const { data } = await axios.post(
        "https://sixpackproject.onrender.comapi/v1/admin/product/new",
        product,
        config
      );
...
export const updateProductByAdminById = createAsyncThunk(
  "products/updateProductByAdminById",
  async ({ productId: id, myForm: productUpdateInfo }) => {
    try {
      // get token from cookie and send via get request
      function getCookie(name) {
        const value = `; ${document.cookie}`;
